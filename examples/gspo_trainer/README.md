...
This recipe uses **transformers 4.57.6**, which conflicts with the standard
verl-omni `[gpu]` extra (`kernels==0.14.1` requires `huggingface-hub>=1.10`,
while transformers 4.x requires `huggingface-hub<1.0`). Do **not** run
`uv pip install -e ".[gpu]"` for this recipe — install vLLM directly instead.

This recipe was last validated end-to-end on the following stack
(rollout↔actor pearson ≈ 0.993):
...
```bash
# vLLM + vLLM-Omni rollout backend (install vLLM directly — skip verl-omni [gpu])
pip install vllm==0.22.0
pip install "vllm-omni @ git+https://github.com/vllm-project/vllm-omni.git@v0.22.0"

# Pin transformers to the validated 4.x release (5.x breaks FSDP checkpoint loading).
...
# verl-omni (this repo) — base deps only; do not add [gpu] (kernels conflicts with 4.57.6)
pip install -e .
```
...
>
> **Do not combine with `[gpu]`.** `kernels==0.14.1` (the FA3 actor backend in
> the `[gpu]` extra) requires `huggingface-hub>=1.10`, which conflicts with
> transformers 4.x (`huggingface-hub<1.0`). This Thinker recipe uses SDPA, not
> the Diffusers FA3 kernels path, so skipping `[gpu]` is fine.
