...
# Explicit list: "all-linear" matches nothing on transformers 5.x.
LORA_TARGET_MODULES="[q_proj,k_proj,v_proj,o_proj,gate_proj,up_proj,down_proj]"

# Qwen3-Omni FSDP loading requires transformers 4.x. CI's shared [gpu] install
# resolves transformers 5.x (needed by kernels); downgrade here with --no-deps to
# avoid fighting the kernels/huggingface-hub pin. Test 8 runs last in gpu_smoke.
if ! python3 - <<'PY'
import transformers
raise SystemExit(0 if transformers.__version__.startswith("4.") else 1)
PY
then
    python3 -m pip install --no-deps --force-reinstall "transformers==4.57.6"
    python3 -m pip install --force-reinstall "huggingface-hub>=0.34.0,<1.0"
fi
