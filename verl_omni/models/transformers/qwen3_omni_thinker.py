...
def _register_qwen3_omni_automodel() -> None:
    ...
    # verl's apply_monkey_patch reads num_attention_heads / text_config from the
    # top-level config; Qwen3-Omni nests these under thinker_config.text_config.
    if not hasattr(Qwen3OmniMoeConfig, "_verl_omni_config_compat_patched"):
        ...
        @property
        def _num_key_value_heads(self):
            return self.thinker_config.text_config.num_key_value_heads

        Qwen3OmniMoeConfig.text_config = _text_config
        Qwen3OmniMoeConfig.num_attention_heads = _num_attention_heads
        Qwen3OmniMoeConfig.num_key_value_heads = _num_key_value_heads
        Qwen3OmniMoeConfig._verl_omni_config_compat_patched = True
...
def patch_hf_processor_for_qwen3_omni() -> None:
    ...
    The original uses a ``match`` block that cannot be extended at runtime; try
    the Qwen3-Omni processor path first so verl does not warn/reject the type.
    """
    try:
    ...
    def _setup_qwen3_omni_processor(name_or_path, **kwargs):
    ...
    def _patched_hf_processor(name_or_path, **kwargs):
        # Handle Qwen3-Omni before verl's match block raises on the processor type.
        try:
            qwen3_processor = _setup_qwen3_omni_processor(name_or_path, **kwargs)
            if qwen3_processor is not None:
                return qwen3_processor
        except Exception:
            pass

        return _original_hf_processor(name_or_path, **kwargs)
